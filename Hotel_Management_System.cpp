...
void HOTEL::add()
{
	// system("clear");
	int r,flag;
	ofstream fout("Record.DAT",ios::app|ios::binary);  //to record the data of the room allocated
	cout<<"\n\t\t\t +-----------------------+";
	cout<<"\n\t\t\t | Rooms  |   Room Type  |";
	cout<<"\n\t\t\t +-----------------------+";
	cout<<"\n\t\t\t |   1-50 |    Deluxe    |";
	...
	else
	{
	   ...
	   else
	   {
		   ...
		   fout.write((char*)this, sizeof(HOTEL));  //used to write binary data to a file.
...
{
	system("clear");
	ifstream fin("Record.DAT",ios::in|ios::binary);  //ifstream (Input File Stream) is used to read from files.
	int r,flag;
	cout<<"\n Enter Room Number: ";
	cin>>r;
...
void HOTEL::rooms()
{
	...
	while(fin.read((char*)this,sizeof(HOTEL)))
	{
		...
		cout<<setw(GuestWidth)<<setfill(separator)<<name<<" |";            // to print name in a structured format
...
void HOTEL::modify_name(int r)
{
	...
	while(!file.eof())
	{
		pos=file.tellg();     //tellg() : ifstream that returns the current position of the file pointer in a file
		file.read((char*)this,sizeof(HOTEL));
		if(room_no==r)
		{
			cout<<"\n Enter New Name: ";
			cin>>name;
			file.seekg(pos);                  //moves the file pointer to a specific position
			file.write((char*)this,sizeof(HOTEL));      // Write the HOTEL object at that position
...
void HOTEL::delete_rec()
{
	...
	cout<<"\n Enter Room Number: ";
	cin>>r;
		ifstream fin("Record.DAT",ios::in|ios::binary);  // Open original file for reading
		ofstream fout("temp.DAT",ios::out|ios::binary);   // Open temp file for writing
	while(fin.read((char*)this,sizeof(HOTEL)))
	{
		if(room_no==r)
		{
...
void HOTEL::lunch(int r)
{
	...
	while(!file.eof())
	{
		...
		if(room_no==r)
		{
			pay=1000*num;
			cost=cost+pay;
			file.seekg(pos);        //Moves the read pointer to the position pos in the file.
			file.write((char*)this,sizeof(HOTEL));
			cout<<"\n Amount added to the bill: Rs. "<<pay;
			flag=1;
			break;
